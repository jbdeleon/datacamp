...
get_ipython().run_cell_magic('sql', '', '\n--Counts the number of missing values from both tables\nSELECT COUNT(*)\nFROM game_sales g\nLEFT JOIN reviews r\nON g.game = r.game\nWHERE critic_score IS NULL\nAND user_score IS NULL;')
...
get_ipython().run_cell_magic('sql', '', '\n--Displays the top 10 average critic score and number of games having more than 4 games each year\nSELECT year, ROUND(AVG(critic_score),2) AS avg_critic_score, COUNT(g.game) AS num_games\nFROM game_sales g\nINNER JOIN reviews r\nON g.game = r.game\nGROUP BY year\nHAVING COUNT(g.game) > 4\nORDER BY avg_critic_score DESC\nLIMIT 10;\n')
...
get_ipython().run_cell_magic('sql', '', '\n--This returns the year and average critic score each year\nSELECT year, avg_critic_score\nFROM top_critic_years\nEXCEPT\nSELECT year, avg_critic_score\nFROM top_critic_years_more_than_four_games\nORDER BY avg_critic_score DESC;\n')
...
get_ipython().run_cell_magic('sql', '', '\n\n--This query display the top 10 average user scorees each year and having at more than 4 games each year\nSELECT year, ROUND(AVG(user_score),2) AS avg_user_score, COUNT(*) AS num_games\nFROM game_sales\nINNER JOIN reviews\nUSING(game)\nGROUP BY year\nHAVING COUNT(*) > 4\nORDER BY avg_user_score DESC\nLIMIT 10;')
...
get_ipython().run_cell_magic('sql', '', '\n--This query returns the year that appear in both tables\nSELECT year\nFROM top_critic_years_more_than_four_games\nINTERSECT\nSELECT year\nFROM top_user_years_more_than_four_games')
...
get_ipython().run_cell_magic('sql', '', '\n--This query returns the year and the number of games sold each year\nSELECT year, SUM(games_sold) AS total_games_sold\nFROM game_sales\nWHERE year IN(SELECT year\n            FROM top_critic_years_more_than_four_games\n            INTERSECT\n            SELECT year\n            FROM top_user_years_more_than_four_games)\nGROUP BY year\nORDER BY total_games_sold DESC;')
